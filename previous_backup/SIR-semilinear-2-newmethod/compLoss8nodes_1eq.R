...
ggplot(Allbox, aes(x = alllabel, y = LS ,color = Method)) +
  ...
  scale_x_discrete(name = "Quality of prior information",
        labels=c("NLS1"="High","SLS1"="High",
                 ...
                 "NLS3"="","SLS3"="",
                 "NLS4"="Low","SLS4"="Low")) +
  ggtitle(expression(NLS~vs~SLS~of~"semi-linear SIR"~Model~with~sigma==0.01)) +
  theme(
...
ggsave("../out/nls_vs_sls_semi-linear-sir_sigma-0.01_1eq.pdf", device="pdf")


# variances of parameters estimates
NLS_Lin_var <- c(var(d1$NLSest_beta1_1),var(d2$NLSest_beta1_1),var(d3$NLSest_beta1_1),var(d4$NLSest_beta1_1))
SLS_Lin_var <- c(var(d1$SLSest_beta1_1),var(d2$SLSest_beta1_1),var(d3$SLSest_beta1_1),var(d4$SLSest_beta1_1))
NLS_Nlin_var <- c(var(d1$NLSest_S1_1),var(d2$NLSest_S1_1),var(d3$NLSest_S1_1),var(d4$NLSest_S1_1))
SLS_Nlin_var <- c(var(d1$SLSest_S1_1),var(d2$SLSest_S1_1),var(d3$SLSest_S1_1),var(d4$SLSest_S1_1))
...
ggplot(DFVar, aes(x=PriorInf)) +
  ...
  ggtitle(expression("Ratio of variance of param est. for 1 year 1 group semi-linear SIR model"~sigma==0.01)) +
  theme(plot.title = element_text(hjust = 0.5))
ggsave("../out/variance_ratio_semi-linear-sir_sigma-0.01_1eq.pdf", device="pdf")
