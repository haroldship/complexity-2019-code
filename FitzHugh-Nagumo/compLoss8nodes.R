...
NLS_Lin_mse <- c(mean((d1$NLSest_a - 0.2)^2)+mean((d1$NLSest_b - 0.2)^2),
                 mean((d2$NLSest_a - 0.2)^2)+mean((d2$NLSest_b - 0.2)^2),
                 mean((d3$NLSest_a - 0.2)^2)+mean((d3$NLSest_b - 0.2)^2),
                 mean((d4$NLSest_a - 0.2)^2)+mean((d4$NLSest_b - 0.2)^2))
SLS_Lin_mse <- c(mean((d1$SLSest_a - 0.2)^2)+mean((d1$SLSest_b - 0.2)^2),
                 mean((d2$SLSest_a - 0.2)^2)+mean((d2$SLSest_b - 0.2)^2),
                 mean((d3$SLSest_a - 0.2)^2)+mean((d3$SLSest_b - 0.2)^2),
                 mean((d4$SLSest_a - 0.2)^2)+mean((d4$SLSest_b - 0.2)^2))
NLS_Nlin_mse <- c(mean((d1$NLSest_c - 3)^2),mean((d2$NLSest_c - 3)^2),mean((d3$NLSest_c - 3)^2),mean((d4$NLSest_c - 3)^2))
SLS_Nlin_mse <- c(mean((d1$SLSest_c - 3)^2),mean((d2$SLSest_c - 3)^2),mean((d3$SLSest_c - 3)^2),mean((d4$SLSest_c - 3)^2))
...
MSERatio <- c(SLS_Lin_mse / NLS_Lin_mse, SLS_Nlin_mse / NLS_Nlin_mse)
Linearity <- factor(c(rep("Linear", 4), rep("Non-linear", 4)))
DFVar <- data.frame(PriorInf, Linearity, VarRatio, MSERatio)
...
ggplot(DFVar, aes(x=PriorInf)) +
  geom_point(aes(y=MSERatio, colour=Linearity, shape=Linearity), size=4) +
  scale_colour_discrete(name="Parameter set") +
  scale_shape_discrete(name="Parameter set") +
  scale_x_discrete(name="Quality of prior information", labels=c("1"="High", "2"="", "3"="", "4"="Low")) +
  scale_y_continuous(name=expression(MSE~Ratio~SLS/NLS), limits=c(0,NA)) +
  labs(title="Ratio of MSE of parameter estimates for FitzHugh-Nagumo model",
       subtitle=expression(V==-1~";"~R==0.5)) +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
ggsave("../out/mse_ratio_fitzhugh-nagumo_V-1_R0.5.pdf", device="pdf")
