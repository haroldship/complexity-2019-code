...
nlin_lower <- 0
names(nlin_lower) <- Svars
lower <- rep(0,length(pars))
names(lower) <- pars
nlin_upper <- 1
names(nlin_upper) <- Svars
upper <- c(10, 1)
names(upper) <- pars
...
for(ip in 1:3){
  ...
  results <- foreach(j=1:N, .packages='simode', .export=args) %dorng% {
    ...
    nlin_init <- rnorm(length(S0),S0,priorInf[1]*abs(S0))
    names(nlin_init) <- Svars
    lin_init <- rnorm(length(beta),beta,priorInf[ip]*abs(beta))
    init <- c(lin_init, nlin_init)
    names(init) <- pars
    
    ptimeNLS <- system.time({
      NLSmc <- simode(equations=equations, pars=pars, time=time, obs=obs[Ivars],
                      fixed=c(I0,gamma), nlin=Svars, start=init,
