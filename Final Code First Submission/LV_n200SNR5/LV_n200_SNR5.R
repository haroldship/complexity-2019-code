...
T=25
vars <- c('X','Y')
eq_X <- 'alpha*X-beta*(1+epsilon*sin(2*pi*(t/T+omega)))*X*Y'
eq_Y <- 'delta*(1+epsilon*sin(2*pi*(t/T+omega)))*X*Y-gamma*Y'
...
#lower <- upper <- c(rep(0.5, length(lin_pars)), rep(0, length(nlin_pars)))
lower <- 0.1*c(theta[lin_pars],theta[nlin_pars])
names(lower) <- pars
nlin_lower <- lower[nlin_pars]
upper <- 1.5*c(theta[lin_pars],theta[nlin_pars])
#upper <- c(rep(1.5, length(lin_pars)), rep(1, length(nlin_pars)))
