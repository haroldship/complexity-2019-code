...
nlin_lower <- rep(0.5,length(nlin_pars))
names(nlin_lower) <- nlin_pars
lower <- c(rep(0,4), 0, nlin_lower)
names(lower) <- pars
nlin_upper <- rep(1.5,length(nlin_pars))
names(nlin_upper) <- nlin_pars
upper <- c(rep(10, 4), 4, nlin_upper)
names(upper) <- pars
...
#               lower=nlin_lower, upper=nlin_upper,
...
#                fixed=c(gamma, x0), nlin=nlin_pars, start=init,
